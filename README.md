...
`Browser PCM → Moonshine Tiny Streaming → Gemma 3 1B Q4_K_M → Piper Lessac Medium → Browser audio`
...
- About 2 GB free disk for images and model weights
...
docker compose logs -f model-init llama backend
```
...
docker compose logs -f backend llama piper
...
- `LLAMA_THREADS`: CPU threads assigned to Gemma; start with the physical core count.
- `MAX_TOKENS`: maximum response length; shorter responses reduce total turn time.
- `SYSTEM_PROMPT`: voice assistant behavior.
...
- Moonshine Tiny Streaming is downloaded by the official `moonshine-voice` package.
- Gemma is `ggml-org/gemma-3-1b-it-GGUF`, file `gemma-3-1b-it-Q4_K_M.gguf`.
- Piper uses `en_US-lessac-medium` through the official `piper-tts` downloader.

The Gemma model is subject to its model license; Piper 1 is GPL-3.0-or-later; Moonshine English models and library are MIT.
